...
Point3f operator*(cv::Mat M, const cv::Point3f &p) {
...
int main(int argc, char *argv[]) {
    ...
    // Transform origin to homogenous
    Mat homOrigin(4, 1, CV_64F);
    homOrigin.at<double>(0, 0) = 0.0;
    homOrigin.at<double>(1, 0) = 0.0;
    homOrigin.at<double>(2, 0) = 0.0;
    homOrigin.at<double>(3, 0) = 1.0;


    // Kalman filter
    Mat P = Mat::zeros(4, 4, CV_64F);
    Mat F = Mat::zeros(4, 4, CV_64F);
    Mat Q = Mat::zeros(4, 4, CV_64F);
    Mat R = Mat::zeros(1, 1, CV_64F);
    Mat Z = Mat::zeros(1, 1, CV_64F);
    Mat H = Mat::zeros(1, 4, CV_64F);
    Mat K = Mat::zeros(4, 1, CV_64F);
    R.at<double>(0, 0) = 0.01;
    Q.at<double>(0, 0) = 0.01;
    Q.at<double>(1, 1) = 0.02;
    Q.at<double>(2, 2) = 0.03;
    H.at<double>(0, 0) = 0;
    H.at<double>(0, 1) = 0;
    H.at<double>(0, 2) = 1;
    H.at<double>(0, 3) = 0;
        ...
        while (1) {
            ...
            if (FD_ISSET(s, &tempFds)) {
                ...
                if (fields[0] == "O") {
                    ...
                    xAngle = std::stod(fields[3]);
                    yAngle = std::stod(fields[4]);
                    zAngle = std::stod(fields[5]);
...
//        cv::medianBlur(currImage, currImage, 3);
        ...
        if (prevFeatures.empty() || currFeatures.empty() || transformMatrix.empty())
            continue;

        // Kalman transition matrix
        F.at<double>(0, 0) = transformMatrix.at<double>(0, 0);
        F.at<double>(0, 1) = transformMatrix.at<double>(0, 1);
        F.at<double>(1, 0) = transformMatrix.at<double>(1, 0);
        F.at<double>(1, 1) = transformMatrix.at<double>(1, 1);
        F.at<double>(0, 3) = transformMatrix.at<double>(0, 2);
        F.at<double>(1, 3) = transformMatrix.at<double>(1, 2);
        F.at<double>(2, 2) = 1;
        F.at<double>(3, 3) = 1;
        F.at<double>(2, 3) = std::asin(
                transformMatrix.at<double>(0, 0) > 1.0f ? 1.0f : transformMatrix.at<double>(0, 0));

        // Update homOrigin
        homOrigin = F * homOrigin;

        P = F * P * F.t() + Q;
        K = P * H.t() * (H * P * H.t() + R).inv();
//        std::cout << K << std::endl;
//        std::cout << Z << std::endl;
//        std::cout << homOrigin << std::endl;
//        std::cout << H << std::endl;
////        std::cout << K << std::endl;
        homOrigin = homOrigin + K * (Z - H * homOrigin);
        P = P - K * H * P;
...
//            circle(currImageColored, currFeatures.at(i), 1, CV_RGB(0, 0, 255),
//                   2);
//        }

        circle(trajectory, Point(homOrigin.at<double>(0, 0) / 10 + 300, homOrigin.at<double>(1, 0) / 10 + 100), 1,
               CV_RGB
               (255, 0, 0), 2);
